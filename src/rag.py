...
from src.prompts import get_contextualize_prompt, get_qa_prompt, NOT_FOUND_MESSAGE
...
class RAGOrchestrator:
    ...
    def run(
        ...
        if self.analytics is not None and self.analytics.is_statistical_query(question, self.llm):
    ...
    def _load_analytics():
        if not os.path.exists(PATIENT_CSV_PATH):
            return None
        return PatientAnalytics(PATIENT_CSV_PATH)
